...
export function NoteCard({ note, project }: { note: Note; project?: Project }) {
  return (
    <div className={cn("p-4 rounded-2xl border transition hover:shadow-card bg-surface dark:bg-dark-surface border-border dark:border-dark-border", note.is_pinned && "border-primary/40")}>
      <h3 className="font-bold text-[15px] line-clamp-2">{note.title}</h3>
      ...
      <div className="mt-3 flex items-center gap-2 text-xs text-text-muted dark:text-dark-text-muted">
        {note.is_pinned && <span className="text-primary font-bold">Pinned</span>}
